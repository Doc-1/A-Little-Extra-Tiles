package com.docvin.alet.common.placement.shape.type;
...
public class LittleShapeCenteredCylinder extends LittleShape {

    public LittleShapeCenteredCylinder() {
        super(2);
    }
        ...
        for (int incA = 0; incA < sizeA; incA++) {
            for (int incB = 0; incB < sizeB; incB++) {
                ...
                if (valueA + valueB <= 1) {
                    ...
                    switch (direction) {
                        case 0:
                            toAdd = new LittleBox(min.x + incA, min.y, min.z + incB, min.x + incA + 1, max.y, min.z + incB
                                    + 1);
                            break;
                        case 1:
                            toAdd = new LittleBox(min.x, min.y + incA, min.z + incB, max.x, min.y + incA + 1, min.z + incB
                                    + 1);
                            break;
                        case 2:
                            toAdd = new LittleBox(min.x + incA, min.y + incB, min.z, min.x + incA + 1, min.y + incB
                                    + 1, max.z);
                            break;
                    }
    ...
    public List<GuiControl> getCustomSettings(NBTTagCompound nbt, LittleGridContext context) {
        ...
        controls.add(new GuiStateButton("direction", nbt.getInteger("direction"), 5, 52, "facing: y", "facing: x", "facing: z"));
        return controls;
    }
