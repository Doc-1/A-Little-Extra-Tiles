...
import com.docvin.alet.components.gui.controls.drawing.GuiImage;
...
public class GuiModifibleTextBox extends GuiTextBox {
    ...
    private void positionText() {
        Iterator<Entry<Integer, List<ModifyText>>> itr = map.entrySet().iterator();
        float y = 0;
        while (itr.hasNext()) {
            ...
            for (ModifyText modText : modTextList) {
                ...
                if (modText.imagePath != null) {
                    this.locationImageMap.put(
                            new Float[]{(float) (((double) this.width / 2) - ((modText.imageWidth * modText.scale) / 2)), y + 2},
                            modText);
                    ...
                    if (modText.center)
                        nX = (float) (((double) this.width / 2) - ((font.getStringWidth(modText.text) * modText.scale) / 2));
                ...
                x += (int) (font.getStringWidth(modText.text) * modText.scale);
    ...
    public boolean isMouseOverClickable(int posX, int posY) {
        for (Entry<Float[], ModifyText> entry : this.locationClickableMap.entrySet()) {
            ...
            if (modText.mouseOver && !modText.url.isEmpty()) {
                customTooltip = new ArrayList<>(Collections.singletonList(modText.url));
                break;
            } else
                customTooltip = null;
        }
    ...
    public boolean isMouseOver(int posX, int posY) {
        return isMouseOverClickable(posX, posY);
    }
    ...
    public final static class ModifierAttribute {
        ...
        public static String addDefinitionText(String text) {
            return addText(1.1, 0x00FFFF, false, 0, text, false, true, false);
        }

        public static String addDefinitionText(String text, int line) {
    ...
    private static class ModifyText {
        ...
        public boolean equals(Object obj) {
            ModifyText text = (ModifyText) obj;
    ...
    private static class ModifyTextMap extends LinkedHashMap<Integer, List<ModifyText>> {
