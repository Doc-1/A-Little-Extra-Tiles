...
public class GuiWrappedTextField extends GuiFocusControl {
    ...
    public void writeText(String text) {
        ...
        if (allowedChars != null) {
            for (char aChar : chars) {
                if (ArrayUtils.contains(allowedChars, aChar))
                    s3.append(aChar);
            }
            s2 = s3.toString();
        }

        int i = Math.min(this.cursorPosition, this.selEnd);
        int j = Math.max(this.cursorPosition, this.selEnd);
    ...
    protected void renderContent(GuiRenderHelper helper, Style style, int width, int height) {
        ...
        int i = this.enabled ? enabledColor : disabledColor;
