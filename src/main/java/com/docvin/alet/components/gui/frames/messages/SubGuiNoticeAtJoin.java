package com.docvin.alet.components.gui.frames.messages;
...
import com.docvin.alet.components.gui.controls.labeling.GuiModifibleTextBox;
...
public class SubGuiNoticeAtJoin extends SubGui {
    ...
    public static void findConfigValue(ICreativeConfigHolder holder, ArrayList<String> path) {
        ...
        for (ConfigKey keys : holder.fields()) {
            if (!Objects.equals(keys.name, path.get(0)))
                continue;
            if (path.size() > 1) {
                if (Objects.equals(keys.name, path.get(0)) && keys.get() instanceof ICreativeConfigHolder) {
                    path.remove(0);
    ...
    public void createControls() {
        GuiModifibleTextBox box = (new GuiModifibleTextBox("", GuiModifibleTextBox.ModifierAttribute.addText(1, ColorUtils.WHITE, false, 0,
                "Attention this version of A Little Extra Tiles is in ALPHA. Issues will arise, report them to " + GuiModifibleTextBox.ModifierAttribute
                        .end() + GuiModifibleTextBox.ModifierAttribute.addClickableText(
                        "https://github.com/Doc-1/A-Little-Extra-Tiles/issues") + GuiModifibleTextBox.ModifierAttribute.end() + GuiModifibleTextBox.ModifierAttribute
                        .addDefaultText(
