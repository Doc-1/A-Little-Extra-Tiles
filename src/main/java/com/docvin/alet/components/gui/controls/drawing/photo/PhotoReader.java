...
public class PhotoReader {
    ...
    /**
     * @param img    Image from a website or directory
     * @param height height of the photo
     * @param width  width of the photo
     * @return New resized photo
     */
    ...
    public static BufferedImage getImage(String input, String uploadOption) {
        ...
        try {
            switch (uploadOption) {
                case "Print From Item":
                case "Print From Block":
    ...
    public static int getPixelWidth(String input, String uploadOption) {
        return getImage(input, uploadOption).getWidth();
    }

    public static int getPixelLength(String input, String uploadOption) {
        return getImage(input, uploadOption).getHeight();
    }

    public static boolean photoExists(String input, String uploadOption) {
        return getImage(input, uploadOption) != null;
    }

    /**
     * @param input        The path that the player gives from the SubGuiPhotoImport
     * @param uploadOption True or False if using URL
     * @param grid         The context or grid size of the tile.
     * @return Returns the NBT data for the structure
     */
    public static ItemStack photoToStack(String input, boolean ignoreAlpha, String uploadOption, int grid, SubGui gui, double colorPerc) {
        ...
        if (image != null) {
            ...
            savePreview(previews, stack); // save tiles to itemstacks
            return stack;
        }
    ...
    public static void savePreview(LittlePreviews previews, ItemStack stack) {
        ...
        if (stack == null)
            throw new NullPointerException("ItemStack field is empty");
        ...
        NBTTagList list = writePreviews(previews);
    ...
    public static NBTTagList writePreviews(LittlePreviews previews) {
        ...
        for (ArrayList<LittlePreview> littlePreviews : groups.values()) {
            while (!littlePreviews.isEmpty()) {
                LittlePreview grouping = littlePreviews.remove(0);
                NBTTagCompound groupNBT = null;

                for (Iterator<LittlePreview> iterator2 = littlePreviews.iterator(); iterator2.hasNext(); ) {
