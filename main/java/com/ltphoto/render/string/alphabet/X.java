...
public class X extends DrawCharacter {
	
	public X(Facing facing, BufferBuilder bufferbuilder, RenderWorldLastEvent event) {
		
	}
	
	public X(Vec3d start, Facing facing, double scale, BufferBuilder bufferbuilder, RenderWorldLastEvent event) {
		this.scale = scale;
		this.facing = facing;
	...
	public ArrayList<Vec3d> vecList() {
		...
		vec.add(new Vec3d(0, 0, 0.08));
		vec.add(new Vec3d(0.08, 0, 0));
		vec.add(new Vec3d(0.04, 0, 0.04));
		vec.add(new Vec3d(0.08, 0, 0.08));
