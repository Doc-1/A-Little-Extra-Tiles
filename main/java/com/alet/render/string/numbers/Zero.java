...
public class Zero extends DrawCharacter {
	
	public Zero(Facing facing, BufferBuilder bufferbuilder, RenderWorldLastEvent event) {
		
	}
	
	public Zero(Vec3d start, Facing facing, double scale, BufferBuilder bufferbuilder, RenderWorldLastEvent event) {
		this.scale = scale;
		this.facing = facing;
	...
	public ArrayList<Vec3d> vecList() {
		...
		vec.add(new Vec3d(0.04, 0, 0));
		vec.add(new Vec3d(0.025, 0, 0.002));
		vec.add(new Vec3d(0.06, 0, 0.045));
