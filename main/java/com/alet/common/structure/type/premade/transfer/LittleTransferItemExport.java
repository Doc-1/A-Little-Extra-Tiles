			...
			if (this.getParent().getStructure() instanceof LittleStorage) {
				...
				for (int i = 0; i < storage.inventorySize; i++) {
					if (!storage.inventory.getStackInSlot(i).getItem().equals(Items.AIR)) {
						...
						EntityItem item = new EntityItem(this.getWorld(), this.getStructureLocation().pos.getX() + x, this.getStructureLocation().pos.getY() + y, this.getStructureLocation().pos.getZ() + z, stack);
						item.motionX = 0;
						item.motionY = 0;
						item.motionZ = 0;
