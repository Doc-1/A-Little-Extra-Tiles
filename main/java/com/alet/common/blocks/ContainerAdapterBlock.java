...
public class ContainerAdapterBlock extends BlockContainer {
    
    public static final PropertyDirection FACING = BlockHorizontal.FACING;
    public TESignalConverter te = new TESignalConverter();
    
    public ContainerAdapterBlock(String name) {
        super(Material.IRON);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote)
            System.out.println(te.getNetwork());
        return true;
    }
    ...
    public static int getPowerOnSide(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        ...
        if (iblockstate.canProvidePower())
            if (block == Blocks.REDSTONE_BLOCK)
                ...
                return block == Blocks.REDSTONE_WIRE ? iblockstate.getValue(BlockRedstoneWire.POWER).intValue() : worldIn
                        .getStrongPower(pos, side);
        else
            return 0;
    }
