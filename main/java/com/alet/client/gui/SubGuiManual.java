...
public class SubGuiManual extends SubGui {
    ...
    public BufferedImage testing() {
        ...
        org.commonmark.node.Node document = parser
                .parse("<html> <body style=\"background-color:rgb(100, 100, 100);\"> <h1>Heading</h1> <p>This is a paragraph.sdfsdjfn sdjf hsajdfb ljsdfbl ajhsbf hjsdbfn ajkfngdsb fghjhnjkds hjbn</p> </body> </html>");
    ...
    public void createControls() {
        ...
        GuiImage image = new GuiImage("image", testing(), 0, 0);
        panelRight.addControl(new GuiLabel("Hello", 0, 0));
        scrollBoxLeft.addControl(image);
        panelLeft.controls.add(scrollBox);
        
        List<GuiTreePart> listOfMenus = new ArrayList<GuiTreePart>();
        drawshape.addMenu(box).addMenu(slices).addMenu(polygon).addMenu(wall).addMenu(pillar).addMenu(curves).addMenu(cylinder).addMenu(sphere).addMenu(pyramid).addMenu(tile)
                .addMenu(type).addMenu(connected).addMenu(centeredAlet).addMenu(magicWandAlet);
        ...
        staticTypes.addMenu(fixedType).addMenu(ladderType).addMenu(bedType).addMenu(chairType).addMenu(storageType).addMenu(noClipType).addMenu(lightType).addMenu(messageType)
                .addMenu(alwaysOnLightType).addMenu(ropeConnectorType);
    ...
    public void getWelcomeLTMsg() {
        scrollBoxLeft.addControl(new GuiModifibleTextBox("t", ModifierAttribute.scale(2) + ModifierAttribute.bold() + "Welcome To LittleTiles" + ModifierAttribute
                .end() + ModifierAttribute.scale(1) + ModifierAttribute
                        .newLines(2) + "    LittleTiles is a mod for anyone who wants to push the limit of building in Minecraft. This mod allows you to build with what are called " + ModifierAttribute
                                .end() + ModifierAttribute.scale(1.1) + ModifierAttribute.italic() + ModifierAttribute.color(0x00FFFF) + "Tiles." + ModifierAttribute
                                        .end() + ModifierAttribute
                                                .scale(1) + " Tiles are smaller than a vanilia Minecraft block, with the default minimal size being 1/32nd the size of a vanilia Minecraft block. You can place, remove, and modify tiles in several different sizes called " + ModifierAttribute
                                                        .end() + ModifierAttribute.scale(1.1) + ModifierAttribute.italic() + ModifierAttribute
                                                                .color(0x00FFFF) + "Grid Size." + ModifierAttribute.end() + ModifierAttribute
                                                                        .scale(1) + " By default the grid sizes avaliable for you to use are as follows: 1, 2, 4, 8, 16," + " and 32. You can increase the smallest tile you can use in the configuration file. To learn how go to " + ModifierAttribute
                                                                                .end() + ModifierAttribute.italic() + ModifierAttribute.underline() + ModifierAttribute
                                                                                        .scale(1) + ModifierAttribute.clickable() + "Configuration" + ModifierAttribute
                                                                                                .end() + ModifierAttribute
                                                                                                        .scale(1) + ", under LittleTiles." + ModifierAttribute.end(), 0, 0, 350) {
    ...
    public void getDrawShape() {
        scrollBoxLeft.addControl(new GuiModifibleTextBox("", ModifierAttribute.addText(2, ColorUtils.WHITE, false, 0, "Draw Shapes", false, true, false) + ModifierAttribute
                .addText(1, ColorUtils.WHITE, false, 2, "    A ", false, false, false) + ModifierAttribute
                        .addText(1.1, 0x00FFFF, false, 0, "Draw Shape", true, false, false) + ModifierAttribute
                                .addText(1, ColorUtils.WHITE, false, 0, ", is the type of shape that the little tools can use. The draw shape can remove,add or edit tiles, depending on the tool and placement mode that is used.", false, false, false), 0, 0, 350));
    }
    
    public void getDrawShapeBox() {
        scrollBoxLeft.addControl(new GuiModifibleTextBox("", ModifierAttribute.addText(2, ColorUtils.WHITE, false, 0, "Draw Shape: Box", false, true, false) + ModifierAttribute
                .addText(1, ColorUtils.WHITE, false, 2, "    The Box draw shape is a simple shape. As the name implies this draw shape allows you draw a squares, rectangles, or cubes. To draw with this shape right click to set point A then right click again to set point B. This will then place the cube.", false, false, false) + ModifierAttribute
    ...
    public void getDrawShapeSlice() {
        scrollBoxLeft.addControl(new GuiModifibleTextBox("", ModifierAttribute.addText(2, ColorUtils.WHITE, false, 0, "Draw Shape: Slice", false, true, false) + ModifierAttribute
                .addText(1, ColorUtils.WHITE, false, 2, "    The Slice draw shape allows you to draw a slice or slope." + " To draw with this shape right click to set point A then right click again to set point B. This will then place the slice. You can use the arrow keys to change the" + " facing of the slope.", false, false, false), 0, 0, 350));
        scrollBoxLeft.addControl(new GuiGIF("", "assets/alet/gif/slice.gif", 0, 65, 1));
        scrollBoxLeft
                .addControl(new GuiModifibleTextBox("", ModifierAttribute.addText(2, ColorUtils.WHITE, false, 0, "Draw Shape: Slice Corner", false, true, false) + ModifierAttribute
    ...
    public void getLogicGateMsg() {
        scrollBoxLeft.addControl(new GuiModifibleTextBox("", ModifierAttribute.addText(2, ColorUtils.WHITE, false, 0, "Logic Gates", false, true, false) + ModifierAttribute
    ...
    public void getAndMsg() {
        scrollBoxLeft.addControl(new GuiModifibleTextBox("", ModifierAttribute.addText(2, ColorUtils.WHITE, false, 0, "Logic Gate: And", false, true, false) + ModifierAttribute
