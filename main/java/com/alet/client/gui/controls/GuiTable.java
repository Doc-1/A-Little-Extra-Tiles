	...
	protected void renderContent(GuiRenderHelper helper, Style style, int width, int height) {
		...
		for (int i = 0; i < rows; i++)
			for (int j = 0; j < columns; j++) {
				...
				font.drawString(text, (j * cellWidth)
				        + ((cellWidth / 2) - (font.getStringWidth(text) / 2)), (i * cellHeight)
				                + (font.FONT_HEIGHT / 2), ColorUtils.WHITE, true);
				helper.drawLine(j * cellWidth, i * cellHeight, (j * cellWidth), (i * cellHeight)
				        + cellHeight, ColorUtils.WHITE);
